    ...
    I: Interner,
{
                ...
                self.assemble_and_merge_candidates(
                    ...
                    |ecx| {
                        ...
                        for arg in goal.predicate.alias.own_args(cx).iter() {
                            let Some(term) = arg.into_term() else {
                                continue;
                            };
                            match ecx.structurally_normalize_term(goal.param_env.clone(), term) {
                                Ok(term) => {
                                    if term.is_infer() {
                                        return Some(
                    ...
                    |ecx| {
                        ecx.probe(|result: &QueryResult<I>| ProbeKind::RigidAlias {
                            result: result.clone(),
                        })
                        .enter(|this| {
    ...
    pub fn instantiate_normalizes_to_term(
        ...
        self.eq(goal.param_env.clone(), &goal.predicate.term, &term)
    ...
    I: Interner,
{
    ...
    fn match_assumption(
        ...
        ecx.add_goals(
            ...
            cx.own_predicates_of(goal.predicate.def_id())
                .iter_instantiated(cx, goal.predicate.alias.args.as_slice())
    ...
    fn consider_builtin_future_candidate(
        ...
        let term = args.clone().as_coroutine().return_ty().into();
    ...
    fn consider_builtin_iterator_candidate(
        ...
        let term = args.clone().as_coroutine().yield_ty().into();
        ...
        ecx.probe_builtin_trait_candidate(BuiltinImplSource::Misc).enter(|ecx| {
            ...
            let wrapped_expected_ty = Ty::new_adt(
                ...
                cx.mk_args(&[Ty::new_adt(
                    ...
                    cx.mk_args(&[expected_ty.clone().into()]),
                )
                .into()]),
            );
            let yield_ty = args.clone().as_coroutine().yield_ty();
        ...
        Ok(if target_container_def_id == impl_trait_ref.def_id.into() {
            ...
            self.add_goals(
                ...
                cx.predicates_of(target_container_def_id)
                    .iter_instantiated(cx, target_args.as_slice())
                    .map(|pred| goal.with(cx, pred)),
            );
