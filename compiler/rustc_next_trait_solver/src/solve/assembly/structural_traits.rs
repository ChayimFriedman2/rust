    ...
    I: Interner,
{
    ...
    match ty.kind() {
        ...
        ty::RawPtr(element_ty, _) | ty::Ref(_, element_ty, _) => {
            Ok(ty::Binder::dummy(vec![element_ty.to_owned()]))
        }
        ...
        ty::Tuple(tys) => {
            ...
            Ok(ty::Binder::dummy(tys.r().iter().map(MyToOwned::o).collect()))
        }

        ty::Closure(_, args) => {
            Ok(ty::Binder::dummy(vec![args.r().as_closure().tupled_upvars_ty().o()]))
        }

        ty::CoroutineClosure(_, args) => {
            Ok(ty::Binder::dummy(vec![args.r().as_coroutine_closure().tupled_upvars_ty().o()]))
        }

        ty::Coroutine(def_id, args) => Ok(ty::Binder::dummy(vec![
            args.r().as_coroutine().tupled_upvars_ty().o(),
        ...
        ty::CoroutineWitness(def_id, args) => Ok(ecx
            ...
            .instantiate(cx, args.r())
            .map_bound(|bound| bound.types.r().iter().map(MyToOwned::o).collect())),

        ty::UnsafeBinder(bound_ty) => Ok(bound_ty.map_bound_ref(|ty| vec![ty.clone()])),

        // For `PhantomData<T>`, we pass `T`.
        ty::Adt(def, args) if def.is_phantom_data() => {
            Ok(ty::Binder::dummy(vec![args.r().type_at(0).o()]))
        }

        ty::Adt(def, args) => {
            Ok(ty::Binder::dummy(def.all_field_tys(cx).iter_instantiated(cx, args.r()).collect()))
        }
        ...
        ty::Alias(ty::Opaque, ty::AliasTy { def_id, args, .. }) => {
            ...
            Ok(ty::Binder::dummy(vec![cx.type_of(*def_id).instantiate(cx, args.r())]))
        }
    }
}
    ...
    I: Interner,
{
    match ty.kind() {
        ...
        ty::Tuple(tys) => {
            Ok(ty::Binder::dummy(tys.r().last().map_or_else(Vec::new, |ty| vec![ty.o()])))
        }
        ...
        ty::Adt(def, args) => {
            if let Some(crit) = def.sizedness_constraint(ecx.cx(), sizedness) {
                Ok(ty::Binder::dummy(vec![crit.instantiate(ecx.cx(), args.r())]))
            } else {
    ...
    I: Interner,
{
    match ty.kind() {
        ...
        ty::Tuple(tys) => Ok(ty::Binder::dummy(tys.r().iter().map(MyToOwned::o).collect())),
    ...
    match self_ty.kind() {
        ...
        ty::FnDef(def_id, args) => {
            ...
            if sig.skip_binder_ref().is_fn_trait_compatible() && !cx.has_target_features(*def_id) {
                Ok(Some(
                    sig.instantiate(cx, args.r()).map_bound(|sig| {
                        (Ty::new_tup(cx, sig.inputs().as_slice()), sig.output().o())
                    }),
                ))
            } else {
                Err(NoSolution)
        ...
        ty::Closure(_, args) => {
            let closure_args = args.r().as_closure();
            ...
            Ok(Some(
                closure_args
                    .sig()
                    .map_bound(|sig| (sig.inputs().get(0).unwrap().o(), sig.output().o())),
            ))
        }
        ...
        ty::CoroutineClosure(def_id, args) => {
            let args = args.r().as_coroutine_closure();
            ...
            Ok(Some(args.coroutine_closure_sig().rebind((sig.tupled_inputs_ty.o(), coroutine_ty))))
        }
...
pub(in crate::solve) fn extract_tupled_inputs_and_output_from_async_callable<I: Interner>(
    ...
    match self_ty.kind() {
        ty::CoroutineClosure(def_id, args) => {
                ...
                && !args.tupled_upvars_ty().is_ty_var()
            {
                ...
                nested.push(
                    ty::TraitRef::new(
                        ...
                        [kind_ty.o(), Ty::from_closure_kind(cx, goal_kind)],
                    )
                    .upcast(cx),
                );
            ...
            Ok((
                args.coroutine_closure_sig().rebind(AsyncCallableRelevantTypes {
                    tupled_inputs_ty: sig.tupled_inputs_ty.o(),
                    output_coroutine_ty: coroutine_ty,
                    coroutine_return_ty: sig.return_ty.o(),
                }),
                nested,
            ))
        }
        ...
        ty::Closure(_, args) => {
            ...
            let mut nested = vec![
                bound_sig
                    .rebind(ty::TraitRef::new(cx, future_trait_def_id, [sig.output().o()]))
                    .upcast(cx),
            ];
            ...
            let future_output_ty = Ty::new_projection(cx, future_output_def_id, [sig.output().o()]);
            Ok((
                bound_sig.rebind(AsyncCallableRelevantTypes {
                    tupled_inputs_ty: sig.inputs().get(0).unwrap().o(),
                    output_coroutine_ty: sig.output().o(),
...
fn fn_item_to_async_callable<I: Interner>(
    ...
    let nested = vec![
        bound_sig.rebind(ty::TraitRef::new(cx, future_trait_def_id, [sig.output().o()])).upcast(cx),
    ];
    let future_output_def_id = cx.require_lang_item(SolverLangItem::FutureOutput);
    let future_output_ty = Ty::new_projection(cx, future_output_def_id, [sig.output().o()]);
    Ok((
...
fn coroutine_closure_to_certain_coroutine<I: Interner>(
    cx: I,
    goal_kind: ty::ClosureKind,
    goal_region: I::RegionRef<'_>,
    def_id: I::CoroutineClosureId,
    args: ty::CoroutineClosureArgs<'_, I>,
    sig: ty::CoroutineClosureSignature<'_, I>,
) -> I::Ty {
...
fn coroutine_closure_to_ambiguous_coroutine<I: Interner>(
    ...
    let tupled_upvars_ty = Ty::new_projection(
        ...
        [
            I::GenericArg::from(args.kind_ty().o()),
            Ty::from_closure_kind(cx, goal_kind).into(),
            goal_region.into(),
            sig.tupled_inputs_ty.o().into(),
            args.tupled_upvars_ty().o().into(),
            args.coroutine_captures_by_ref_ty().o().into(),
        ],
    );
    sig.to_coroutine(
        cx,
        args.parent_args(),
        Ty::from_closure_kind(cx, goal_kind).r(),
        cx.coroutine_for_closure(def_id),
        tupled_upvars_ty.r(),
    )
}
...
pub(in crate::solve) fn const_conditions_for_destruct<I: Interner>(
    ...
    match self_ty.kind() {
        ...
        ty::Adt(adt_def, args) => {
            ...
            match adt_def.destructor(cx) {
                ...
                Some(AdtDestructorKind::Const) => {
                    ...
                    let drop_trait_ref = ty::TraitRef::new(cx, drop_def_id, [self_ty.o()]);
        ...
        ty::Tuple(tys) => Ok(tys
            .r()
            .iter()
            .map(|field_ty| ty::TraitRef::new(cx, destruct_def_id, [field_ty.o()]))
            .collect()),
...
pub(in crate::solve) fn predicates_for_object_candidate<D, I>(
    ...
    object_bounds: I::BoundExistentialPredicatesRef<'_>,
    ...
    I: Interner,
{
    ...
    requirements.extend(elaborate::elaborate(
        ...
        cx.explicit_super_predicates_of(trait_ref.def_id)
            .iter_instantiated(cx, trait_ref.args.r())
            .map(|(pred, _)| pred),
    ));
    ...
    for associated_type_def_id in cx.associated_type_def_ids(trait_ref.def_id) {
        ...
        requirements.extend(
            cx.item_bounds(associated_type_def_id).iter_instantiated(cx, trait_ref.args.r()),
        );
    }

    let mut replace_projection_with: HashMap<_, Vec<_>> = HashMap::default();
    for bound in object_bounds.iter_ref() {
        if let ty::ExistentialPredicate::Projection(proj) = bound.skip_binder_ref() {
            ...
            let proj = proj.with_self_ty(cx, trait_ref.self_ty().o());
            replace_projection_with
                .entry(proj.def_id())
                .or_default()
                .push(bound.rebind(proj.clone()));
        }
    }

    let mut folder = ReplaceProjectionWith {
        ecx,
        param_env: param_env.reborrow(),
    ...
    Ok(folder
        .nested
        .into_iter()
        .chain(requirements.into_iter().map(|clause| Goal::new(cx, param_env.o(), clause)))
        .collect())
}

struct ReplaceProjectionWith<'a, 'b, I: Interner, D: SolverDelegate<Interner = I>> {
    ecx: &'a mut EvalCtxt<'b, D>,
    param_env: I::ParamEnvRef<'a>,
    self_ty: I::TyRef<'a>,
    ...
    I: Interner,
{
    ...
    fn try_eagerly_replace_alias(
        ...
        if alias_term.self_ty() != self.self_ty.reborrow() {
            return Ok(None);
        }
