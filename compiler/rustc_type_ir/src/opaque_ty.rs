...
impl<I: Interner> OpaqueTypeKey<I> {
    pub fn iter_captured_args(&self, cx: I) -> impl Iterator<Item = (usize, I::GenericArg)> {
        ...
        std::iter::zip(self.args.iter(), I::variances_into_iter(variances)).enumerate().filter_map(
    ...
    pub fn fold_captured_lifetime_args(
        ...
        let Self { def_id, ref args } = *self;
        let variances = cx.variances_of(def_id.into());
        let args = std::iter::zip(args.iter(), variances.iter()).map(|(arg, v)| {
            match (arg.clone().kind(), v) {
